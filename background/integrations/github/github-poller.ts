...
async function readState(): Promise<GitHubState> {
  try {
    const parsed = JSON.parse(await Bun.file(STATE_PATH).text()) as Partial<GitHubState>;
    return {
...
async function runGhJson(ghBin: string, args: string[], description: string): Promise<GhQueryResult> {
  try {
    ...
    if (!Array.isArray(parsed)) {
      log('warn', `${description} returned non-array JSON`);
      return { ok: false, items: [] };
    }
...
async function main(): Promise<void> {
  ...
  for (const repo of repos) {
    const mergedResult = await runGhJson(ghBin, [
    ...
    const releaseResult = await runGhJson(ghBin, [
    ...
    for (const raw of releaseResult.items) {
      ...
      context.releases.push({ ...release, repo });
      context.new_release_tags.push(id);
    }

    const openPrResult = await runGhJson(ghBin, [
  ...
  log('info', `fetched: ${context.merged_prs.length} merged PRs, ${context.releases.length} releases, ${context.open_prs.length} open PRs`);

  if (context.new_pr_ids.length === 0 && context.new_release_tags.length === 0) {
    if (!context.advance_watermark) {
      log('warn', 'critical GitHub query failed — preserving previous poll watermark');
      return;
    }
    await advancePollWatermark(polledAt);
    log('info', 'no new GitHub activity since last poll');
    return;
  }

  const jobPath = join(PENDING_DIR, `job-github-${crypto.randomUUID()}.json`);
  writeJsonDurable(jobPath, {
    source: 'github',
    profile,
    timestamp: polledAt,
  ...
  log('info', `poll complete (state acknowledgment deferred until synthesis succeeds, advance_watermark=${context.advance_watermark})`);
}
