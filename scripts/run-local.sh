...
for key in SUPABASE_URL SUPABASE_PUBLISHABLE_KEY SUPABASE_SECRET_KEY FLY_API_TOKEN FLY_APP_NAME FLY_SANDBOX_IMAGE FLY_REGION SANDBOX_CALLBACK_SECRET GITHUB_APP_ID GITHUB_APP_SLUG GITHUB_APP_PRIVATE_KEY GITHUB_APP_WEBHOOK_SECRET BETTER_AUTH_DATABASE_URL BETTER_AUTH_SECRET; do
  require_env_var "$key"
done
...
landing_env=(
  ...
  "DRAFT_API_BASE_URL=http://localhost:$DRAFT_BACKEND_PORT"
